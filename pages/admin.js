          ...
          <div className="mt-3 space-y-2">
            {(cfg.categories || []).map((c, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center">
                <input className="input" placeholder="id" value={c.id} onChange={(e)=>update("categories",(arr)=>{arr[idx].id=e.target.value})}/>
                <input className="input" placeholder="Nome" value={c.name} onChange={(e)=>update("categories",(arr)=>{arr[idx].name=e.target.value})}/>
                <button className="btn" onClick={()=>update("categories",(arr)=>arr.splice(idx,1))}>Excluir</button>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>update("categories",(arr)=>arr.push({id:"",name:""}))}>+ Adicionar categoria</button>
          </div>
        </section>
          ...
          <div className="mt-3 space-y-2">
            {(cfg.addons || []).map((a, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
                <input className="input" placeholder="id" value={a.id} onChange={(e)=>update("addons",(arr)=>{arr[idx].id=e.target.value})}/>
                <input className="input" placeholder="Nome" value={a.name} onChange={(e)=>update("addons",(arr)=>{arr[idx].name=e.target.value})}/>
                <input type="number" className="input" placeholder="Preço" value={a.price} onChange={(e)=>update("addons",(arr)=>{arr[idx].price=Number(e.target.value||0)})}/>
                <button className="btn" onClick={()=>update("addons",(arr)=>arr.splice(idx,1))}>Excluir</button>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>update("addons",(arr)=>arr.push({id:"",name:"",price:0}))}>+ Adicionar adicional</button>
          </div>
        </section>
              ...
              <div key={idx} className="rounded-xl border border-[#e5e7eb] p-3 bg-[#fafafa]">
                <div className="grid md:grid-cols-2 gap-3">
                  <div className="grid gap-2">
                    ...
                    <input className="input" value={p.id} onChange={(e)=>update("products",(arr)=>{arr[idx].id=e.target.value})}/>
                    <label className="text-sm">Categoria (id)</label>
                    <input className="input" value={p.category} onChange={(e)=>update("products",(arr)=>{arr[idx].category=e.target.value})}/>
                    <label className="text-sm">Nome</label>
                    <input className="input" value={p.name} onChange={(e)=>update("products",(arr)=>{arr[idx].name=e.target.value})}/>
                    <label className="text-sm">Descrição</label>
                    <textarea className="input h-20" value={p.desc||""} onChange={(e)=>update("products",(arr)=>{arr[idx].desc=e.target.value})}/>
                  </div>
                  <div className="grid gap-2">
                  ...
                  <div className="mt-2 space-y-2">
                    {(p.sizes || []).map((s, sidx) => (
                      <div key={sidx} className="grid grid-cols-[1fr_2fr_1fr_auto] gap-2 items-center">
                        <input className="input" placeholder="code" value={s.code} onChange={(e)=>update("products",(arr)=>{arr[idx].sizes[sidx].code=e.target.value})}/>
                        <input className="input" placeholder="label" value={s.label} onChange={(e)=>update("products",(arr)=>{arr[idx].sizes[sidx].label=e.target.value})}/>
                        <input type="number" className="input" placeholder="price" value={s.price} onChange={(e)=>update("products",(arr)=>{arr[idx].sizes[sidx].price=Number(e.target.value||0)})}/>
                        <button className="btn" onClick={()=>update("products",(arr)=>{arr[idx].sizes.splice(sidx,1)})}>Remover</button>
                      </div>
                    ))}
                    <button className="btn" onClick={()=>update("products",(arr)=>{arr[idx].sizes = arr[idx].sizes||[]; arr[idx].sizes.push({code:"",label:"",price:0})})}>+ Adicionar tamanho</button>
                  </div>
                </div>

                <div className="mt-3 flex justify-between">
                  <button className="btn" onClick={()=>update("products",(arr)=>arr.splice(idx,1))}>Excluir produto</button>
                </div>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>update("products",(arr)=>arr.push({id:"",category:"",name:"",desc:"",price:0,img:"",tags:[],sizes:[]}))}>+ Adicionar produto</button>
          </div>
        </section>
