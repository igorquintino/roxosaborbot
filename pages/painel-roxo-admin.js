...
export default function AdminPage() {
  ...
  const [cfg, setCfg] = useState({
    brand: { name: "", logoText: "", colors: { primary: "#6D28D9", primaryDark: "#4C1D95", accent: "#22C55E" } },
    store: { whatsapp: "", instagram: "", deliveryHours: "", bannerUrl: "", logoUrl: "", closedNote: "", raspadinhaCopy: "" },
    categories: [],
    addons: [],
    products: [],
    coupons: {},
  ...
  function downloadJson() {
    ...
    a.href = url; a.download = "rs_config_v1.json"; a.click();
  ...
  function uploadJson(e) {
    ...
    reader.onload = () => {
      try { setCfg(JSON.parse(String(reader.result))); alert("✅ Config carregada (salve para aplicar)!"); }
      ...
      <main className="max-w-5xl mx-auto p-4 grid lg:grid-cols-2 gap-6">
          ...
          <div className="mt-3 grid gap-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input type="color" className="input" value={cfg.brand.colors.primary} onChange={(e)=>setCfg({...cfg, brand:{...cfg.brand, colors:{...cfg.brand.colors, primary:e.target.value}}})}/>
              </div>
              <div>
                <label className="text-sm">Primária (dark)</label>
                <input type="color" className="input" value={cfg.brand.colors.primaryDark} onChange={(e)=>setCfg({...cfg, brand:{...cfg.brand, colors:{...cfg.brand.colors, primaryDark:e.target.value}}})}/>
              </div>
              <div>
                <label className="text-sm">Accent</label>
                <input type="color" className="input" value={cfg.brand.colors.accent} onChange={(e)=>setCfg({...cfg, brand:{...cfg.brand, colors:{...cfg.brand.colors, accent:e.target.value}}})}/>
              </div>
            </div>
            ...
            <input className="input" value={cfg.store.closedNote || ""} onChange={(e)=>setCfg({...cfg, store:{...cfg.store, closedNote:e.target.value}})} />
          ...
          <div className="mt-3 space-y-2">
            {(cfg.categories || []).map((c, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center">
                <input className="input" placeholder="id" value={c.id} onChange={(e)=>updateArray("categories", (arr)=>{arr[idx].id=e.target.value})}/>
                <input className="input" placeholder="Nome" value={c.name} onChange={(e)=>updateArray("categories", (arr)=>{arr[idx].name=e.target.value})}/>
                <div className="flex gap-2">
                  <button className="btn" onClick={()=>updateArray("categories",(arr)=>arr.splice(idx,1))}>Excluir</button>
                </div>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>updateArray("categories",(arr)=>arr.push(structuredClone(EMPTY_CATEGORY)))}>+ Adicionar categoria</button>
          </div>
        </section>
          ...
          <div className="mt-3 space-y-2">
            {(cfg.addons || []).map((a, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
                <input className="input" placeholder="id" value={a.id} onChange={(e)=>updateArray("addons", (arr)=>{arr[idx].id=e.target.value})}/>
                <input className="input" placeholder="Nome" value={a.name} onChange={(e)=>updateArray("addons", (arr)=>{arr[idx].name=e.target.value})}/>
                <input type="number" className="input" placeholder="Preço" value={a.price} onChange={(e)=>updateArray("addons", (arr)=>{arr[idx].price=Number(e.target.value||0)})}/>
                <button className="btn" onClick={()=>updateArray("addons",(arr)=>arr.splice(idx,1))}>Excluir</button>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>updateArray("addons",(arr)=>arr.push(structuredClone(EMPTY_ADDON)))}>+ Adicionar adicional</button>
          </div>
        </section>
              ...
              <div key={idx} className="rounded-xl border border-[#e5e7eb] p-3 bg-[#fafafa]">
                <div className="grid md:grid-cols-2 gap-3">
                  <div className="grid gap-2">
                    ...
                    <input className="input" value={p.id} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].id=e.target.value})}/>
                    <label className="text-sm">Categoria (id)</label>
                    <input className="input" value={p.category} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].category=e.target.value})}/>
                    <label className="text-sm">Nome</label>
                    <input className="input" value={p.name} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].name=e.target.value})}/>
                    <label className="text-sm">Descrição</label>
                    <textarea className="input h-20" value={p.desc || ""} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].desc=e.target.value})}/>
                  </div>
                  <div className="grid gap-2">
                    <label className="text-sm">Preço base</label>
                    <input type="number" className="input" value={p.price} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].price=Number(e.target.value||0)})}/>
                    <label className="text-sm">Imagem (URL/DataURL)</label>
                    <input className="input" value={p.img} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].img=e.target.value})}/>
                    <label className="text-sm">Tags (vírgula)</label>
                    <input className="input" value={(p.tags||[]).join(",")} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].tags=e.target.value.split(",").map(s=>s.trim()).filter(Boolean)})}/>
                  </div>
                </div>
                  ...
                  <div className="mt-2 space-y-2">
                    {(p.sizes || []).map((s, sidx) => (
                      <div key={sidx} className="grid grid-cols-[1fr_2fr_1fr_auto] gap-2 items-center">
                        <input className="input" placeholder="code" value={s.code} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].sizes[sidx].code=e.target.value})}/>
                        <input className="input" placeholder="label" value={s.label} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].sizes[sidx].label=e.target.value})}/>
                        <input type="number" className="input" placeholder="price" value={s.price} onChange={(e)=>updateArray("products",(arr)=>{arr[idx].sizes[sidx].price=Number(e.target.value||0)})}/>
                        <button className="btn" onClick={()=>updateArray("products",(arr)=>{arr[idx].sizes.splice(sidx,1)})}>Remover</button>
                      </div>
                    ))}
                    <button className="btn" onClick={()=>updateArray("products",(arr)=>{arr[idx].sizes = arr[idx].sizes||[]; arr[idx].sizes.push({code:"",label:"",price:0})})}>+ Adicionar tamanho</button>
                  </div>
                </div>

                <div className="mt-3 flex justify-between">
                  <button className="btn" onClick={()=>updateArray("products",(arr)=>arr.splice(idx,1))}>Excluir produto</button>
                </div>
              </div>
            ))}
            <button className="btn-primary" onClick={()=>updateArray("products",(arr)=>arr.push(structuredClone(EMPTY_PRODUCT)))}>+ Adicionar produto</button>
          </div>
        </section>
        ...
        <section className="rounded-2xl bg-white border border-[#e5e7eb] p-4 shadow-sm">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={clearAll} className="btn">Limpar tudo</button>
          </div>
        </section>
      </main>
...
function CouponEditor({ cfg, setCfg }) {
  ...
  function addCoupon() {
    ...
    setCfg((prev) => {
      const next = structuredClone(prev);
      if (!next.coupons) next.coupons = {};
      next.coupons[code.toUpperCase()] = { type: "percent", value: 10, label: "10% de desconto aplicado" };
      return next;
    });
  }
      ...
      {entries.map(([code, c]) => (
        <div key={code} className="rounded-xl border border-[#e5e7eb] p-3 bg-[#fafafa]">
          ...
          <div className="grid md:grid-cols-3 gap-2 mt-2">
            ...
            <input className="input" type="number" placeholder="valor (%)" value={c.value || 0} onChange={(e)=>setField(code,"value",Number(e.target.value||0))} />
            <input className="input" placeholder="label" value={c.label || ""} onChange={(e)=>setField(code,"label",e.target.value)} />
          </div>
          <div className="mt-2">
