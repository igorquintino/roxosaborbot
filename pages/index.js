...
const DEFAULT_CONFIG = {
  ...
  store: {
    ...
    closedNote: "Loja fechada • Abre amanhã às 09:00",
    raspadinhaCopy:
...
export default function RoxoSaborMenu() {
  ...
  }, [category, query, cfg.products]);

  const subtotal = cart.reduce((s, i) => s + i.subtotal, 0);
  const discount =
    couponInfo?.type === "percent" ? (subtotal * (couponInfo.value || 0)) / 100 : 0;
  ...
  function addToCart(product, { size, addons = [], qty = 1, obs = "" } = {}) {
    ...
    const subtotal = (Number(basePrice || 0) + addonsTotal) * qty;
    ...
    <div className="min-h-screen text-[15px] bg-[--bg] text-[--fg]">
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-[color:var(--line)]">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center gap-3">
          <a
            href="/painel-roxo-admin"
            className="px-3 py-1.5 rounded-xl border border-[color:var(--line)] bg-white hover:bg-[color:var(--chip)]"
          >
            ⚙️ Admin
          </a>
          <div className="mx-auto text-center">
            <div className="text-sm text-[color:var(--muted)]">{cfg.brand.name}</div>
            <div className="text-xs text-[color:var(--muted)]">{cfg.store.deliveryHours}</div>
          </div>
          <a
            href="#carrinho"
      ...
      <section className="max-w-md mx-auto">
        <div className="relative overflow-hidden">
          <img src={cfg.store.bannerUrl} className="h-28 w-full object-cover" alt="" />
          <div className="mx-4 -mt-10 rounded-3xl bg-white p-4 shadow-md border border-[color:var(--line)]">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h1 className="text-lg font-semibold">{cfg.brand.logoText}</h1>
      ...
      <section className="max-w-md mx-auto px-4 pt-4">
        ...
        <div className="mt-4 text-sm rounded-2xl bg-white border border-[color:var(--line)] p-4 shadow-sm">
          ...
          <p className="text-[color:var(--muted)]">{cfg.store.raspadinhaCopy}</p>
      ...
      <main className="max-w-md mx-auto px-2 pb-20">
        ...
        <div className="rounded-2xl bg-white border border-[color:var(--line)] shadow-sm">
          {filtered.map((p, idx) => (
            <ProductRow key={p.id} product={p} first={idx === 0} onClick={() => openSheet(p)} />
          ))}
        </div>
      </main>
                    ...
                    <li key={i.id} className="py-3 flex gap-3 items-start">
                      ...
                      <div className="grow">
                        <div className="font-medium leading-tight">
                          {i.name}
                          {i.size ? (
                            <span className="text-[color:var(--muted)]"> ({i.size.label})</span>
                          ) : null}
                          {i.qty ? (
                            <span className="text-[color:var(--muted)]"> × {i.qty}</span>
                          ) : null}
                        </div>
                        {i.addons?.length ? (
                        ...
                        {i.obs ? (
                          <div className="text-xs text-[color:var(--muted)]">Obs: {i.obs}</div>
                        ) : null}
      ...
      <footer className="py-10 text-center text-xs text-[color:var(--muted)]">
        <div>
          {cfg.brand.name} • {cfg.store.deliveryHours}
        </div>
      ...
      <style jsx global>{`
        :root {
          ...
          --primaryDark: ${cfg.brand.colors.primaryDark};
...
function ItemSheet({ open, onClose, product, addons, onConfirm }) {
  ...
  const selAddons = (addons || []).filter((a) => picked.has(a.id));
  const addonsTotal = selAddons.reduce((s, a) => s + Number(a.price || 0), 0);
  const price = (Number(base || 0) + addonsTotal) * qty;
